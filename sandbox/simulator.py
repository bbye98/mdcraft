...
from mdcraft.utility.topology import convert_cell_representation
...
lattice_parameters = np.array((30.0, 40.0, 50.0, 30.0, 60.0, 90.0))
positions_nm = rng.random((n_particles, 3), np.float64) * lattice_parameters[:3]
cutoff_nm = 7.5
...
# MDCraft
# mdcraft_nl = build_neighbor_list(positions_nm, cutoff_nm, lattice_parameters)
# mdcraft_pairs = []
# for pid in range(n_particles):
#     mdcraft_pairs.extend([(pid, j) for j in mdcraft_nl[pid]])
# mdcraft_pairs = np.array(mdcraft_pairs, dtype=np.uint32)
