...
def center_of_charge(group: mda.AtomGroup = None,
                    grouping: str = None,
                    *,
                    charges: Union[np.ndarray[float], list[np.ndarray[float]]] = None,
    ...
    # necessary
    missing = (charges is None, positions is None)
    if any(missing):
        if group is None:
            emsg = (
        ...
        # Get particle charges and ensure correct dimensionality, if
        # necessary
        if same:
            if missing[0]:
            ...
            if grouping is not None or n_groups is not None:
                ...
                charges = charges.reshape(shape[:-1])
                positions = positions.reshape(shape)
        else:
            if missing[0]:
                charges = [g.atoms.charges for g in groups]
            if missing[1]:
        ...
        try:
            ...
            pass
        if type(charges) is not type(positions):
            emsg = (
                "The shapes of the arrays containing the charges "
    ...
    if isinstance(positions, np.ndarray):
        ...
        if n_groups is not None:
            charges = charges.reshape((n_groups, -1))
            positions = positions.reshape((n_groups, -1, 3))

        # dipole = sum(q_i * r_i) and should be a vector of shape (3,)
        coc = np.einsum("...a,...ad->...d", np.abs(charges), positions) / np.abs(charges).sum(
        ...
        coc = np.array([
            np.sum(np.abs(c) * p, axis=0) / np.sum(np.abs(c)) for c, p in zip(charges, positions)
        ])

    # Return raw charges and positions, if desired
    if raw:
        return coc, charges, positions
...
def molecular_dipole(group: mda.AtomGroup = None,
    ...
    if isinstance(positions, np.ndarray):
        ...
        # dipole = sum(q_i * r_i) and should be a vector of shape (3,)
        dipole = np.einsum("ij,ijk->ik", charges, positions)

    else:
        if images is not None:
            for j, (p, i) in enumerate(zip(positions, images)):
                positions[j] = p + i * dimensions
        dipole = np.array([np.sum(c*p, axis=0) for c, p in zip(charges, positions)])

    # Return raw charges and positions, if desired
    if raw:
        return dipole, charges, positions

    return dipole

def molecular_quadrupole(group: mda.AtomGroup = None,
                    grouping: str = None,
                    *,
                    charges: Union[np.ndarray[float], list[np.ndarray[float]]] = None,
                    positions: Union[np.ndarray[float], list[np.ndarray[float]]] = None,
    ...
    # Get particle charges and positions from the trajectory, if
    # necessary
    missing = (charges is None, positions is None)
    if any(missing):
        if group is None:
    ...
    # Calculate the quadrupole for the specified grouping
    if isinstance(positions, np.ndarray):
        ...
        # quadrupole = sum_l q_l(3*r_il*r_jl - |r_l|^2*delta_ij)
        quadrupole = np.zeros((positions.shape[0], 3, 3), dtype=float)
        rl = np.sum(np.linalg.norm(positions, axis=2)**2 * charges, axis=1)
        for i in range(3):
            quadrupole[:, i, i] = 3 * np.sum(
                        charges * positions[:, :, i] * positions[:, :, i], axis=1
                    ) - rl
            for j in range(i+1, 3):
                    quadrupole[:, i, j] = np.sum(
                        charges * (3 * positions[:, :, i] * positions[:, :, j]), axis=1
                    )
                    quadrupole[:, j, i] = quadrupole[:, i, j]

    else:
        if images is not None:
            for j, (p, i) in enumerate(zip(positions, images)):
                positions[j] = p + i * dimensions
        quadrupole = np.zeros((len(positions), 3, 3), dtype=float)
        rl = np.sum(np.linalg.norm(positions, axis=2)**2 * charges, axis=1)
        for i in range(3):
            quadrupole[:, i, i] = np.sum(
                        charges * positions[:, :, i] * positions[:, :, i], axis=1
                    ) - rl
            for j in range(i+1, 3):
                    quadrupole[:, i, j] = np.sum(
    ...
    # Return raw charges and positions, if desired
    if raw:
        return quadrupole, charges, positions

    return quadrupole

class PotentialProfile(DynamicAnalysisBase):
    """
    ...
    **kwargs
    ...
    >>> from mdcraft.analysis.profile import PotentialProfile
    ...
    The `PotentialProfile` class can be instantiated with the selected atom-groups and the axes to calculate the density profiles along (charges can also be provided to obtain the charge density profiles):
    
    >>> prof = PotentialProfile([ag1, ag2], groupings="residues", axes="x", charges=[1, -1])
    >>> prof.run()
    ...
    def __init__(
        ...
        dipole_axes: Union[int, str, tuple[Union[int, str]]] = "xyz",
        dipole_center: str = "center_of_mass",
        ...
        **kwargs,
        ...
        if isinstance(dipole_axes, int):
            self._dipole_axis_indices = np.array((dipole_axes,), dtype=int)
        else:
            self._dipole_axis_indices = np.fromiter(
                (ord(ax.lower()) - 120 if isinstance(ax, str) else ax for ax in dipole_axes),
                count=len(dipole_axes),
                dtype=int,
            )
        self.dipole_axes = tuple(chr(120 + i) for i in self._dipole_axis_indices)
    ...
    def _prepare(self) -> None:
        ...
        if not self._parallel:
            ...
            self._dipoles = np.empty((self._N, 3), dtype=float)
            self._quadrupoles = np.empty((self._N, 3, 3), dtype=float)
            ...
            self.results.dipoles = Hash(
                {(ax, ax_d): np.zeros((*shape, n)) for ax, n in zip(self.axes, self._n_bins) for ax_d in self.dipole_axes}
            )
            self.results.quadrupoles = Hash(
                {(ax, ax_d1, ax_d2): np.zeros((*shape, n)) for ax, n in zip(self.axes, self._n_bins) for ax_d1 in self.dipole_axes for ax_d2 in self.dipole_axes}
            )
    ...
    def _single_frame(self) -> None:
        ...
        for ag, gr, s in zip(self._groups, self._groupings, self._slices):
            if self._dipole_center == "center_of_mass":
                self._positions[s] = (
                    ag.positions if gr == "atoms" else center_of_mass(ag, gr)
                )
    ...
    def _conclude(self) -> None:
        ...
        if self._parallel:
            ...
            for ax in self.axes:
                ...
                for ax_d in self.dipole_axes:
                    self.results.dipoles[ax, ax_d] = np.stack(
                        [r[1][ax, ax_d] for r in self._results], axis=1
                    )
                    if self._average:
                        self.results.dipoles[ax, ax_d] = self.results.dipoles[
                            ax, ax_d
                        ].sum(axis=1)
        ...
        for ax, n in zip(self.axes, self._n_bins):
            ...
            for ax_d in self.dipole_axes:
                self.results.dipoles[ax, ax_d] /= denom
    ...
    def calculate_surface_charge_densities(
        ...
        *,
        ...
        if not hasattr(self.results, "charge_densities"):
            emsg = (
                "Either call PotentialProfile.run() before "
                "PotentialProfile.calculate_potential_profiles() or "
