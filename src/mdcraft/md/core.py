...
class State:
    def __init__(
        ...
        *,
        ...
        self._dimensions_nm = np.asarray(
            strip_unit(dimensions, "nm")[0], np.float64
        )
        ...
        if n_particles is None:
            ...
            self._positions_nm = np.asarray(
                strip_unit(positions, "nm")[0], np.float64
            )
        else:
            self._n_particles = n_particles
            self._positions_nm = (
                rng.random((self._n_particles, 3), np.float64)
                * self._dimensions_nm
            )
