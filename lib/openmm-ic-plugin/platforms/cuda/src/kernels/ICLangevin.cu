...
/**
 * Perform the first step of ICLangevin integration.
 */

extern "C" __global__ void integrateICLangevinPart1(
    int numAtoms, int paddedNumAtoms, mixed4* __restrict__ velm,
    const long long* __restrict__ force, mixed4* __restrict__ posDelta,
    const mixed* __restrict__ paramBuffer, const mixed2* __restrict__ dt,
    ...
    while (index < numAtoms) {
        ...
        if (velocity.w != 0) {
            ...
            velocity.y = vscale * velocity.y +
                         fscale * velocity.w * force[index + paddedNumAtoms] +
...
/**
 * Perform the second step of ICLangevin integration.
 */
...
extern "C" __global__ void selectICLangevinStepSize(
    int numAtoms, int paddedNumAtoms, mixed maxStepSize, mixed errorTol,
    mixed friction, mixed kT, mixed2* __restrict__ dt,
    const mixed4* __restrict__ velm, const long long* __restrict__ force,
    mixed* __restrict__ paramBuffer) {
    // Calculate the error.
    ...
    while (index < numAtoms) {
        mixed3 f = make_mixed3(scale * force[index],
                               scale * force[index + paddedNumAtoms],
                               scale * force[index + paddedNumAtoms * 2]);
    ...
    if (blockIdx.x * blockDim.x + threadIdx.x == 0) {
        ...
        if (oldStepSize > 0.0f)
            newStepSize = min(
                newStepSize,
                oldStepSize *
                    2.0f);  // For safety, limit how quickly dt can increase.
        if (newStepSize > oldStepSize && newStepSize < 1.1f * oldStepSize)
            newStepSize =
                oldStepSize;  // Keeping dt constant between steps improves the
                              // behavior of the integrator.
        if (newStepSize > maxStepSize) newStepSize = maxStepSize;
        dt[0].y = newStepSize;
    ...
    __syncthreads();
    if (threadIdx.x < MaxParams) paramBuffer[threadIdx.x] = params[threadIdx.x];
}
